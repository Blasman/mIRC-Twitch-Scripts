...
alias -l slot_emotes {
  :emotes
  $input(Input five emotes that you would like to use for !slot:,eo,Required Input,%slot.emotes)
  IF (!$!) { ECHO You need to input five emotes for !slot! | GOTO emotes }
  ELSE {
    VAR %emotes $!
    IF ($numtok(%emotes,32) != 5) GOTO emotes
...
alias -l slot_houseedge {
  :houseedge
  $input(Be careful with this setting! IF THIS CONFUSES YOU $+ $chr(44) JUST LEAVE THIS SET TO 0. Specify the "house edge" for !slot? $chr(40) $+ from -100 to 100 percent $+ $chr(41) Decimal places are okay. See the documentation on the GitHub wiki for more information.,eo,Required Input,%slot.houseedge)
