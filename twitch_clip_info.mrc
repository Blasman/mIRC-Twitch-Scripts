...
ON $*:TEXT:/clips\.twitch\.tv\/(\w+)/iS:%mychan: {
  ...
  IF (!$JSONError) {
    ...
    ELSE {
      ...
      MSG $chan 📽 Twitch Clip Info linked by $nick ▌ Streamer: %tc_streamer ▌ Clip Title: $json(get_twitch_clip $+ %tc, data, 0, title).value ▌ Game: $twitch_lookup_game($json(get_twitch_clip $+ %tc, data, 0, game_id).value) ▌ Clipper: $json(get_twitch_clip $+ %tc, data, 0, creator_name).value ▌ %created_at $IIF(%tc_view_count, ▌ View Count: $json(get_twitch_clip $+ %tc, data, 0, view_count).value, $null) $IIF(%tc_repost_link, ▌ Link: $json(get_twitch_clip $+ %tc, data, 0, url).value, $null)
    }
  }
